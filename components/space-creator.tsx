import { createSpaceAction } from "@/app/(app)/spaces/actions";
...
export function SpaceCreator() {
  return (
    <section className="rounded-2xl border border-border bg-surface p-6 shadow-sm">
      <h2 className="font-heading text-base font-semibold text-heading">Create a new space</h2>
      <p className="mt-1 text-sm text-body">
        Start with a space for your personal costs, shared household, business, or another financial context.
      </p>

      <form action={createSpaceAction} className="mt-5 flex flex-col gap-4 sm:flex-row sm:items-end">
        <label className="grid flex-1 gap-1.5 text-sm">
          <span className="font-medium text-heading">Space name</span>
          <input
            name="name"
            required
        ...
        <button className="flex items-center justify-center gap-2 rounded-xl bg-heading px-5 py-3 text-sm font-semibold text-on-primary shadow-sm transition hover:bg-heading/90">
          <Plus size={16} />
          Create
        </button>
      </form>
