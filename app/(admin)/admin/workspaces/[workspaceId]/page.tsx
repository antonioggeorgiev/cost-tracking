...
export default async function AdminWorkspaceDetailPage({ params }: WorkspaceDetailPageProps) {
  const { workspaceId } = await params;
  const caller = await getServerCaller();
  const workspace = await caller.admin.getWorkspaceDetail({ workspaceId });

  if (!workspace) notFound();

  const totalSpend = workspace.totalSpendMinor / 100;

  return (
    <div className="space-y-6">
      <div>
        <Link href={routes.adminWorkspaces} className="inline-flex items-center gap-1.5 text-sm text-body hover:text-heading mb-4">
          <ArrowLeft size={14} />
          Back to Workspaces
        </Link>
        <p className="text-xs font-semibold uppercase tracking-[0.15em] text-primary">Workspace Detail</p>
        <h1 className="mt-1 font-heading text-2xl font-bold text-heading">{workspace.name}</h1>
      </div>
      ...
      <section className="rounded-2xl border border-border bg-surface p-6 shadow-sm">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div>
            ...
            <p className="mt-1 font-mono text-sm text-heading">{workspace.slug}</p>
          </div>
          <div>
            <p className="text-xs text-body">Base Currency</p>
            <p className="mt-1 text-sm font-medium text-heading">{workspace.baseCurrencyCode}</p>
          </div>
          <div>
            <p className="text-xs text-body">Created By</p>
            <p className="mt-1 text-sm text-heading">{workspace.createdBy}</p>
          </div>
          <div>
            <p className="text-xs text-body">Created</p>
            <p className="mt-1 text-sm text-heading">{format(workspace.createdAt, "MMM d, yyyy")}</p>
          </div>
        </div>
      </section>
        ...
        <div className="rounded-2xl border border-border bg-surface p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-heading text-lg font-bold text-heading">{workspace.members.length}</p>
            </div>
          </div>
        </div>
        <div className="rounded-2xl border border-border bg-surface p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-heading text-lg font-bold text-heading">{workspace.expenseCount}</p>
            </div>
          </div>
        </div>
        <div className="rounded-2xl border border-border bg-surface p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-heading text-lg font-bold text-heading">
                {totalSpend.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} {workspace.baseCurrencyCode}
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="rounded-2xl border border-border bg-surface shadow-sm">
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          ...
          <span className="text-sm text-body">{workspace.members.length} members</span>
        </div>
