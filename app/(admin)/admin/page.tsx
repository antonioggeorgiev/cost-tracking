...
export default async function AdminDashboardPage() {
  ...
  const [stats, recentUsers, recentWorkspaces] = await Promise.all([
    caller.admin.stats(),
    caller.admin.listUsers({ perPage: 5 }),
    caller.admin.listWorkspaces({ perPage: 5 }),
  ]);

  const statCards = [
    { label: "Total Users", value: stats.userCount, icon: Users, href: routes.adminUsers },
    { label: "Total Workspaces", value: stats.workspaceCount, icon: Building2, href: routes.adminWorkspaces },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <section className="rounded-2xl border border-border bg-surface shadow-sm">
          <div className="flex items-center justify-between border-b border-border px-6 py-4">
            <h2 className="font-heading text-base font-semibold text-heading">Recent Workspaces</h2>
            <Link href={routes.adminWorkspaces} className="text-sm font-medium text-primary hover:underline">View all</Link>
          </div>
          <div className="divide-y divide-border">
            {recentWorkspaces.items.map((ws) => (
              <Link
                key={ws.id}
                href={routes.adminWorkspace(ws.id)}
                className="flex items-center gap-3 px-6 py-3 transition hover:bg-surface-secondary"
              >
