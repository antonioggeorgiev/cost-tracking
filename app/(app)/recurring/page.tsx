...
import { createRecurringTemplateAction, markFixedAsPaidAction, recordVariableRecurringExpenseAction } from "@/app/(app)/recurring/actions";
...
export default async function RecurringPage({ searchParams }: RecurringPageProps) {
  ...
  if (!spaceSlug) {
    ...
    const allTemplates = await caller.recurring.listAllSpaces();
    ...
    return (
      <div className="space-y-8">
        ...
        <div>
          ...
          <p className="mt-2 text-sm text-body">Recurring templates across all your spaces.</p>
        </div>

        {allTemplates.length > 0 ? (
          <section className="rounded-2xl border border-border bg-surface shadow-sm">
            <div className="hidden border-b border-border px-6 py-3 sm:grid sm:grid-cols-[1fr_80px_100px_80px_120px] sm:gap-4">
              <span className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Template</span>
              ...
              <span className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Frequency</span>
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 truncate text-xs text-muted-foreground">
                      {template.category?.name ?? "Uncategorized"} · {template.kind === "fixed_amount" ? "Fixed" : "Variable"}
                    </p>
                  </div>
                  ...
                  <div className="mt-1 sm:mt-0 sm:text-right">
                    {template.workspaceAmountMinor != null ? (
                      ...
                      <p className="text-sm text-body">Variable</p>
                    )}
                  </div>
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border border-border bg-surface py-16 text-center shadow-sm">
            ...
            <p className="mt-4 font-heading text-lg font-semibold text-heading">No recurring templates</p>
            <p className="mt-1 text-sm text-body">Create recurring templates in a specific space to see them here.</p>
          </div>
        )}
      </div>
    );
  ...
  const caller = await getServerCaller();
  await caller.recurring.generateDue({ spaceSlug });
  ...
  const [workspace, categories, templates, dueVariableTemplates, expenseResult] = await Promise.all([
    ...
    caller.recurring.list({ spaceSlug }),
    caller.recurring.dueVariable({ spaceSlug }),
    caller.expenses.list({ spaceSlug }),
  ]);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        {canManage && (
          <Link
            href={`${routes.recurring}?modal=add-recurring`}
