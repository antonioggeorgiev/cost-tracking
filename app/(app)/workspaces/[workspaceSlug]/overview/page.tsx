...
import {
  ...
  TrendingUp, TrendingDown, ArrowRight, AlertCircle, Clock, Calendar,
} from "lucide-react";
...
export default async function OverviewPage({ params }: OverviewPageProps) {
  ...
  const [workspace, data] = await Promise.all([
    ...
    caller.reporting.overview({ workspaceSlug }),
  ]);
  ...
  const sections = [
    ...
    },
    {
      ...
      href: routes.workspaceRecurring(workspace.slug),
    },
    {
      label: "Debts",
      icon: Landmark,
      ...
      href: routes.workspaceDebts(workspace.slug),
    },
  ];
    ...
    <>
      ...
      <section className="rounded-2xl border border-border bg-surface p-6 shadow-sm">
        <div className="flex items-center justify-between">
          ...
          <p className="text-xs font-semibold uppercase tracking-[0.15em] text-body">{workspace.name}</p>
        </div>
            ...
            {data.dueVariableRecurring.map((item) => (
              <div key={item.id} className="flex items-center gap-4 py-3 first:pt-0 last:pb-0">
                ...
                {item.paymentUrl ? (
                  ...
                  <Link href={routes.workspaceRecurring(workspace.slug)} className="text-xs font-medium text-primary hover:underline">
                    Record
                  </Link>
                )}
              </div>
