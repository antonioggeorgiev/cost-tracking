...
export default async function MembersPage({ searchParams }: MembersPageProps) {
  ...
  if (!spaceSlug) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="mt-2 text-sm text-body">Select a space to manage members.</p>
      </div>
    );
  }
  ...
  const data = await caller.members.list({ spaceSlug });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.15em] text-primary">{data.space.name}</p>
          <h1 className="mt-1 font-heading text-2xl font-bold text-heading">Members</h1>
          <p className="mt-2 text-sm text-body">Manage space access and invite new members.</p>
        </div>
