...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const spaceSlug = formData.get("spaceSlug") as string | null;

  if (!file || !spaceSlug) {
    return NextResponse.json({ error: "Missing file or spaceSlug" }, { status: 400 });
  }
  ...
  try {
    ...
    const categories = await caller.categories.list({ spaceSlug });
    ...
    const result = await documentExtractionService.extractRecurringTemplate(base64, file.type, categoryTree);
